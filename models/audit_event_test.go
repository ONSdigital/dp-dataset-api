...
func TestNewAuditEvent(t *testing.T) {
	testCases := []struct {
	...
	}{
		{
			...
			dataset:     nil,
			version:     nil,
			expectedErr: errors.New("either dataset or version must be provided, but not both"),
		},
		{
	...
	Convey("NewAuditEvent input validation", t, func() {
		for _, tc := range testCases {
			Convey(tc.name, func() {
				_, err := NewAuditEvent(tc.requestedBy, tc.action, tc.resource, tc.dataset, tc.version)
	...
	Convey("NewAuditEvent creates AuditEvent correctly", t, func() {
		...
		auditEvent, err := NewAuditEvent(requestedBy, action, resource, dataset, nil)
		So(err, ShouldBeNil)
