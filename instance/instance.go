...
func (s *Store) GetList(w http.ResponseWriter, r *http.Request, limit, offset int) (results interface{}, totalCount int, err error) {
	...
	results, totalCount, err = func() ([]*models.Instance, int, error) {
		...
		if s.EnableURLRewriting {
			...
			err = utils.RewriteInstances(ctx, instancesResults, datasetLinksBuilder, codeListLinksBuilder, importLinksBuilder, s.URLBuilder.GetDownloadServiceURL())
			if err != nil {
...
func (s *Store) Get(w http.ResponseWriter, r *http.Request) {
	...
	if s.EnableURLRewriting {
		...
		err = utils.RewriteInstances(ctx, []*models.Instance{instance}, datasetLinksBuilder, codeListLinksBuilder, importLinksBuilder, s.URLBuilder.GetDownloadServiceURL())
		if err != nil {
