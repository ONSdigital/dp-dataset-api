...
// GetVersionsStaticByEditionNoLimit retrieves all versions for a given dataset and edition without pagination.
func (m *Mongo) GetVersionsStaticByEditionNoLimit(ctx context.Context, datasetID, edition, state string) ([]*models.Version, int, error) {
	selector := buildVersionsQuery(datasetID, edition, state)
