...
func TestGetVersionsReturnsError(t *testing.T) {
	...
	Convey("When the api cannot connect to datastore return an internal server error", t, func() {
		...
		// The resolve returns the error before permission attributes or the dataset type are read.
	...
	Convey("When the dataset does not exist return status not found", t, func() {
		...
		So(len(mockedDataStore.GetDatasetCalls()), ShouldEqual, 1)
		So(len(mockedDataStore.GetDatasetByPreviousSeriesIDCalls()), ShouldEqual, 1)
		So(len(mockedDataStore.GetDatasetTypeCalls()), ShouldEqual, 0)
	})
...
func TestGetVersionReturnsError(t *testing.T) {
	...
	Convey("When the dataset does not exist for return status not found", t, func() {
		...
		mockedDataStore := &storetest.StorerMock{
			...
			// The dataset ID does not match a previous series ID either.
			GetDatasetByPreviousSeriesIDFunc: func(ctx context.Context, previousSeriesID string) (*models.DatasetUpdate, error) {
				return nil, errs.ErrDatasetNotFound
			},
		}
...
func TestGetVersionRecordsAuditEvent(t *testing.T) {
	...
	Convey("Given an authorised request to get a static dataset version", t, func() {
		...
		staticDatasetServiceMock := &applicationMocks.StaticDatasetServiceMock{
			GetVersionPrivateFunc: func(ctx context.Context, datasetID, editionID string, versionID int) (*models.Version, error) {
				return version, nil
			},
		}
		...
		api := GetAPIWithCMDMocks(mockedDataStore, &mocks.DownloadsGeneratorMock{}, authorisationMock, application.SearchContentUpdatedProducer{}, &cloudflareMocks.ClienterMock{}, auditServiceMock, staticDatasetServiceMock, nil, &filesAPISDKMocks.ClienterMock{})
		...
		Convey("When we call the GET version endpoint", func() {
			...
			Convey("And the relevant calls have been made", func() {
				So(len(mockedDataStore.GetVersionsStaticByEditionNoLimitCalls()), ShouldEqual, 1)
				So(len(mockedDataStore.GetDatasetTypeCalls()), ShouldEqual, 1)
				So(len(staticDatasetServiceMock.GetVersionPrivateCalls()), ShouldEqual, 1)
			})
		})
	})
}
...
func TestGetVersionDoesNotRecordAuditEventForUnauthorisedUser(t *testing.T) {
	...
	Convey("Given an unauthorised request to get a static dataset version", t, func() {
		...
		staticDatasetServiceMock := &applicationMocks.StaticDatasetServiceMock{
			GetVersionPublicFunc: func(ctx context.Context, datasetID, editionID string, versionID int) (*models.Version, error) {
				return version, nil
			},
		}
		...
		Convey("When we call the GET version endpoint", func() {
			...
			Convey("And the relevant calls have been made", func() {
				So(len(mockedDataStore.GetVersionsStaticByEditionNoLimitCalls()), ShouldEqual, 1)
				So(len(mockedDataStore.GetDatasetTypeCalls()), ShouldEqual, 1)
				So(len(staticDatasetServiceMock.GetVersionPublicCalls()), ShouldEqual, 1)
			})
		})
	})
}
...
func TestGetVersionAuditEventLogsErrorButContinues(t *testing.T) {
	...
	Convey("Given an authorised request where the audit service fails", t, func() {
		...
		Convey("When we call the GET version endpoint", func() {
			...
			Convey("And the expected calls were made", func() {
				So(len(mockedDataStore.GetVersionsStaticByEditionNoLimitCalls()), ShouldEqual, 1)
				So(len(mockedDataStore.GetDatasetTypeCalls()), ShouldEqual, 1)
				So(len(staticDatasetServiceMock.GetVersionPrivateCalls()), ShouldEqual, 1)
				So(len(auditServiceMock.RecordVersionAuditEventCalls()), ShouldEqual, 1)
...
func TestPutVersionForbidden(t *testing.T) {
	...
	Convey("When a request is made to put version that is forbidden", t, func() {
		...
		Convey("Given a valid request is executed", func() {
			api := GetAPIWithCMDMocks(mockedDataStore, generatorMock, authorisationMock, application.SearchContentUpdatedProducer{}, &cloudflareMocks.ClienterMock{}, &applicationMocks.AuditServiceMock{}, &applicationMocks.StaticDatasetServiceMock{}, nil, &filesAPISDKMocks.ClienterMock{})
			api.Router.ServeHTTP(w, r)
...
func TestPutVersionReturnsSuccessfully(t *testing.T) {
	...
	Convey("When state is unchanged", t, func() {
		...
		Convey("Given a valid request is executed", func() {
			api := GetAPIWithCMDMocks(mockedDataStore, generatorMock, authorisationMock, application.SearchContentUpdatedProducer{}, &cloudflareMocks.ClienterMock{}, auditServiceMock, &applicationMocks.StaticDatasetServiceMock{}, nil, &filesAPISDKMocks.ClienterMock{})
			api.Router.ServeHTTP(w, r)
	...
	Convey("When state is set to associated", t, func() {
		...
		Convey("put version with Cantabular type", func() {
			...
			api := GetAPIWithCantabularMocks(mockedDataStore, generatorMock, authorisationMock, &cloudflareMocks.ClienterMock{}, auditServiceMock, &applicationMocks.StaticDatasetServiceMock{})
			api.Router.ServeHTTP(w, r)
...
func TestPutVersionGenerateDownloadsError(t *testing.T) {
	Convey("given download generator returns an error", t, func() {
		...
		Convey("when put version is called with a valid request", func() {
			...
			api := GetAPIWithCMDMocks(mockedDataStore, mockDownloadGenerator, authorisationMock, application.SearchContentUpdatedProducer{}, &cloudflareMocks.ClienterMock{}, auditServiceMock, &applicationMocks.StaticDatasetServiceMock{}, nil, &filesAPISDKMocks.ClienterMock{})
			api.Router.ServeHTTP(w, r)
...
func TestPutVersionReturnsError(t *testing.T) {
	...
	Convey("Given PUT /versions/{id} with a distribution missing format", t, func() {
		...
		api := GetAPIWithCMDMocks(mocked, &mocks.DownloadsGeneratorMock{}, authorisationMock, application.SearchContentUpdatedProducer{}, &cloudflareMocks.ClienterMock{}, auditServiceMock, &applicationMocks.StaticDatasetServiceMock{}, nil, &filesAPISDKMocks.ClienterMock{})
		api.Router.ServeHTTP(w, r)
...
func TestGetVersionsPreviousSeriesID(t *testing.T) {
	t.Parallel()

	newRequest := func() *http.Request {
		r := httptest.NewRequest("GET", "http://localhost:22000/datasets/old-series/editions/678/versions", http.NoBody)
		r.Header.Set("Authorization", "Bearer "+testAuthToken)
		return mux.SetURLVars(r, map[string]string{"dataset_id": "old-series", "edition": "678"})
	}

	authorisationMock := func() *authMock.MiddlewareMock {
	...
	Convey("Given a request for a series ID that has since changed", t, func() {
		r := newRequest()
		w := httptest.NewRecorder()

		results := []models.Version{{Edition: "678", State: models.AssociatedState}}
		...
		mockedDataStore := &storetest.StorerMock{
			...
			GetDatasetByPreviousSeriesIDFunc: func(ctx context.Context, previousSeriesID string) (*models.DatasetUpdate, error) {
				return &models.DatasetUpdate{ID: "new-series", Next: &models.Dataset{ID: "new-series"}}, nil
			},
			GetVersionsStaticByEditionNoLimitFunc: func(ctx context.Context, datasetID, edition, state string) ([]*models.Version, int, error) {
				return nil, 0, nil
			},
		...
		Convey("When getVersions is called", func() {
			...
			Convey("Then the versions are returned", func() {
				So(err, ShouldBeNil)
				So(w.Code, ShouldEqual, http.StatusOK)
				So(list, ShouldResemble, results)
				So(totalCount, ShouldEqual, 1)
			...
			Convey("And all subsequent lookups use the resolved dataset ID", func() {
				So(len(mockedDataStore.GetDatasetByPreviousSeriesIDCalls()), ShouldEqual, 1)
				So(mockedDataStore.GetDatasetByPreviousSeriesIDCalls()[0].PreviousSeriesID, ShouldEqual, "old-series")
				So(mockedDataStore.GetDatasetTypeCalls()[0].DatasetID, ShouldEqual, "new-series")
				So(mockedDataStore.GetVersionsStaticCalls()[0].DatasetID, ShouldEqual, "new-series")
			})
		})
	})

	Convey("Given a request for a series ID that matches neither a current nor a previous series", t, func() {
		r := newRequest()
		w := httptest.NewRecorder()
		...
		Convey("When getVersions is called", func() {
			...
			Convey("Then a not found error is returned without further lookups", func() {
