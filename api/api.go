...
func routes(host, secretKey string, router *mux.Router, dataStore store.DataStore, urlBuilder *url.Builder, downloadGenerator DownloadsGenerator, healthCheckTimeout time.Duration) *DatasetAPI {
	...
	api.router.HandleFunc("/datasets/{id}/editions", api.privateAuth.ManualCheck(api.getEditions)).Methods("GET")
	api.router.HandleFunc("/datasets/{id}/editions/{edition}", api.privateAuth.ManualCheck(api.getEdition)).Methods("GET")
