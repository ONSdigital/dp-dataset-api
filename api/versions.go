...
func (api *DatasetAPI) deleteVersion(w http.ResponseWriter, r *http.Request) {
	...
	authEntityData, err := api.getAuthEntityData(r)
	if err != nil {
		log.Error(ctx, "deleteVersion endpoint: failed to get auth entity data from request", err, logData)
		handleVersionAPIErr(ctx, err, w, logData)
		return
	}
...
func populateNewVersionDoc(currentVersion, originalVersion *models.Version) (*models.Version, error) {
	...
	if version.Downloads == nil {
		...
		if version.Downloads.ZIP == nil && currentVersion.Downloads != nil {
			version.Downloads.ZIP = currentVersion.Downloads.ZIP
		}
	}
