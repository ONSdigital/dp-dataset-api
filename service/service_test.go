	...
	Convey("Having a set of mocked dependencies", t, func() {
		...
		Convey("Given that Checkers cannot be registered", func() {
			...
			Convey("Then service Run fails, but all checks try to register", func() {
				...
				So(hcMockAddFail.AddCheckCalls()[2].Name, ShouldResemble, "Mongo DB")
		...
		Convey("Given that all dependencies are successfully initialised, observation and private endpoints are disabled", func() {
			...
			Convey("Only the checkers for Kafka and MongoDB are registered, and the healthcheck and http server started", func() {
				So(len(hcMock.AddCheckCalls()), ShouldEqual, 2)
				So(hcMock.AddCheckCalls()[0].Name, ShouldResemble, "Kafka Generate Downloads Producer")
