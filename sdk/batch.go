...
// ProcessInConcurrentBatches is a generic method to concurrently obtain some resource in batches and then process each batch
func processInConcurrentBatchesWithQueryParams(getBatch GenericBatchGetter, processBatch GenericBatchProcessor, reqLimit, reqOffset, batchSize, maxWorkers int) (err error) {
	// validate parameters
	if getBatch == nil {
		return errors.New("getBatch function cannot be nil")
	}
	...
	// determine the total number of required calls
	numCalls := reqLimit / batchSize
	if (reqLimit % batchSize) != 0 {
		numCalls++
	}
	...
	for i := 0; i < numCalls; i++ {
		acquire()
		go doProcessBatch(i*batchSize + reqOffset)
	}
